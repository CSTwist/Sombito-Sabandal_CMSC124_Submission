...
class Parser(private val tokens: List<Token>) {
    ...
    fun parseProgram(): Program {
        ...
        val variables = mutableListOf<Decl.VarDecl>()  // top-level set
    ...
    private fun abilityField(): AbilityField {
        return when {
            ...
            match(TokenType.BEHAVIOR) -> {
                ...
                val body = block()
                AbilityField.BehaviorField(body)
            }
            else -> {
    ...
    private fun statement(): Stmt {
        ...
        return when {
            ...
            check(TokenType.IDENTIFIER) -> {
                // Could be stat entry or function call
                val saved = current
                val name = advance()
                if (match(TokenType.COLON)) {
                    // Stat entry
                    ...
                    error(peek(), "Unexpected statement after identifier '${name.lexeme}'.")
    ...
    private fun primary(): Expr {
        if (match(TokenType.NUMBER)) {
            return Expr.Literal(previous().lexeme.toDouble())
        }
