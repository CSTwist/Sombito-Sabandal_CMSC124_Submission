...
class AstPrinter {
    ...
    private fun importToString(decl: Decl.ImportDecl) =
        "(Import ${decl.name.lexeme})"
    ...
    private fun heroStatementToString(stmt: HeroStatement): String = when (stmt) {
        ...
        is HeroStatement.ScalingCall ->
            "(Scaling ${stmt.param1.lexeme} ${stmt.param2.lexeme})"
    ...
    private fun abilityFieldToString(field: AbilityField): String = when (field) {
        is AbilityField.TypeField ->
            "(Type ${field.value.lexeme})"

        is AbilityField.CooldownField ->
            "(Cooldown ${exprToString(field.value)})"

        is AbilityField.ManaCostField ->
            "(ManaCost ${exprToString(field.value)})"

        is AbilityField.RangeField ->
            "(Range ${exprToString(field.value)})"

        is AbilityField.DamageTypeField ->
            "(DamageType ${field.value.lexeme})"

        is AbilityField.BehaviorField ->
            "(Behavior ${blockToString(field.body)})"
    }
    ...
    private fun arenaItemToString(decl: Decl): String = when (decl) {
        ...
        is Decl.ConstDecl ->
            "(Const ${decl.name.lexeme} = ${exprToString(decl.value)})"

        is Decl.VarDecl ->
            varDeclToString(decl)
        ...
        is Decl.StatusEffectDecl ->
            statusEffectToString(decl)

        is Decl.ImportDecl ->
            importToString(decl)

        is Decl.FunctionDecl ->
            "(FunctionDecl ${decl.name.lexeme} (...))"
    }
    ...
    private fun statusEffectFieldToString(field: StatusEffectField): String = when (field) {
        ...
        is StatusEffectField.DurationField ->
            "(Duration ${exprToString(field.value)})"

        is StatusEffectField.OnApplyField ->
            "(OnApply ${blockToString(field.block)})"

        is StatusEffectField.OnTickField ->
            "(OnTick ${blockToString(field.block)})"
    }
    ...
    private fun itemFieldToString(field: ItemField): String = when (field) {
        is ItemField.PropertyField ->
            "(${field.name.lexeme}: ${exprToString(field.value)})"

        is ItemField.EffectField ->
            "(Effect ${blockToString(field.body)})"
    }
    ...
    private fun stmtToString(stmt: Stmt): String = when (stmt) {
        ...
        is Stmt.ExprStmt ->
            "(Expr ${exprToString(stmt.expr)})"

        is Stmt.IfStmt -> buildString {
            append("(If ${exprToString(stmt.condition)} ")
            append(blockToString(stmt.thenBranch))
            if (stmt.elseBranch != null) {
                append(" else ")
                append(blockToString(stmt.elseBranch))
            }
            append(")")
        }
        ...
        is Stmt.ForStmt -> buildString {
            append("(For ")
            append(stmt.initializer?.let { stmtToString(it) } ?: "nil")
            append(" ; ")
            append(stmt.condition?.let { exprToString(it) } ?: "nil")
            append(" ; ")
            append(stmt.increment?.let { stmtToString(it) } ?: "nil")
            append(" ${blockToString(stmt.body)})")
        }

        is Stmt.ReturnStmt ->
            "(Return ${stmt.value?.let { exprToString(it) } ?: "nil"})"

        is Stmt.FunStmt -> buildString {
    ...
    private fun argumentToString(arg: Argument): String = when (arg) {
        is Argument.NamedArg ->
            "${arg.name.lexeme}: ${exprToString(arg.value)}"

        is Argument.PositionalArg ->
            exprToString(arg.value)
    }
    ...
    private fun exprToString(expr: Expr): String = when (expr) {
        ...
        is Expr.Binary ->
            "(${expr.operator.lexeme} ${exprToString(expr.left)} ${exprToString(expr.right)})"

        is Expr.Grouping ->
            "(group ${exprToString(expr.expression)})"
